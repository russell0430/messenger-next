...
const ConversationList: React.FC<ConversationListProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const updateHandler = (conversation: FullConversationType) => {
      setItems((current) =>
        current.map((currentConversation) => ({
          ...currentConversation,
          messages: (currentConversation.id === conversation.id
            ? conversation
            : currentConversation
          ).messages,
        }))
    ...
    const removeHandler = (removedConversation: FullConversationType) => {
      setItems((current) => [
        ...current.filter(
          (conversation) => conversation.id !== removedConversation.id
        ),
      ])
    ...
    // triggered at app\api\conversations\route.ts
    pusherClient.bind("conversation:new", newHandler)
    pusherClient.bind("conversation:update", updateHandler)

    // triggered at app\api\conversations\[conversationId]\route.ts
